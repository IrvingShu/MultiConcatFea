...
def multiprocessing_concat(nProcess, dirList, pathList, saveDir):
    step = len(pathList) / nProcess
    pos = 0
    processList = []
    for i in range(nProcess):
        if i == nProcess - 1:
            para_pathList = pathList[pos:]
        else: 
            para_pathList = pathList[pos:pos+step]
        pos += step
        processList.append(Process(target=concatN_from_list, args=(dirList, para_pathList, saveDir)))
        processList[i].start()
    for i in range(nProcess):
        processList[i].join()
...
def main(args):
    ...
    multiprocessing_concat(nProcess, dirList, featurePathList, saveDir)
    print 'concat done'
    t1 = time.time()
